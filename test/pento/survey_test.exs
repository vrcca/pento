defmodule Pento.SurveyTest do
  ...
  describe "demographics" do
    ...
    test "get_demographic_by_user/1 returns the demographic of the given user" do
      demographic1 = demographic_fixture() |> Repo.preload(:user)
      demographic2 = demographic_fixture() |> Repo.preload(:user)
      assert Survey.get_demographic_by_user(demographic1.user).id == demographic1.id
      assert Survey.get_demographic_by_user(demographic2.user).id == demographic2.id
    end
    ...
    test "update_demographic/2 with valid data updates the demographic" do
      demographic = demographic_fixture()
      update_attrs = %{gender: "other", year_of_birth: 1921}
  ...
  describe "ratings" do
    ...
    test "create_rating/1 with valid data creates a rating" do
      valid_attrs = %{stars: 2, user_id: user_fixture().id, product_id: product_fixture().id}
